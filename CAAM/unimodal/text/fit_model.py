...
if(path.exists('optimal_weights11.h5')):
	model = load_model()
	model.load_weights('optimal_weights11.h5')
...
while(current_epoch_number <= no_of_downward_epochs):
	...
	loss_test = model.evaluate([X_test, X_test_gender], Y_test, batch_size = int(X_test.shape[0]/2))
