...
from text_X_visual.attention_fusion_network.load_data import load_training_data, load_test_data
from text_X_visual.attention_fusion_network.load_model import load_model
...
while(current_epoch_number < no_of_epochs):
	...
	hist = model.fit([training_facial, training_transcript, training_X_gender], training_Y, batch_size = batch_size, epochs = 1)
	mse_train = hist.history['loss'][-1]
	mse_test, mae_test = model.evaluate([test_facial,  test_transcript, test_X_gender], test_Y, batch_size = test_facial.shape[0])

	print(mse_test, mae_test)

	if(mse_test < min_rmse_test):

		min_rmse_test = mse_test
